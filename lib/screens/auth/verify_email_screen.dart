...
class VerifyEmailScreen extends StatefulWidget {
  const VerifyEmailScreen({Key? key, required this.userCredentials}) : super(key: key);
...
class _VerifyEmailScreenState extends State<VerifyEmailScreen> with ContextSize {
  ...
  void initState() {
    ...
    refresher=Timer.periodic(const Duration(seconds: 2), (timer) {
      Provider.of<MyAuthProvider>(context, listen: false).refreshVerificationStatus();
    });
  }
              ...
              child: MaxSizedContainer(
                ...
                child: Column(
                  children: [
                    ...
                    const Spacer(),
                    auth.emailVerified?const SizedBox.shrink():Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Text("Didn't recieve an email? ", style: Theme.of(context)
                                  .textTheme
                                  .labelSmall
                                  ?.copyWith(
                                      color: bodyTextColor.withOpacity(0.6), fontSize: 13),),
                        TextButton(
                            onPressed: () {
                            },
                            child: Text(
                    ...
                    CustomButton(
                      buttonText: auth.emailVerified?'Continue':'Not Verified',
                      backgroundColor: auth.emailVerified?null:Colors.red,
                      onPressed: auth.emailVerified?()async{
                        Navigator.pushReplacement(context, MaterialPageRoute(builder:(context) => FillProfileScreen(credentials: widget.userCredentials),));
                      }:null,
                      fullWidth: true,
                    ),
                    const SizedBox(height: 22,),
                    Text('By continuing you’re indicating that you accept our Terms of Use and our Privacy Policy', textAlign: TextAlign.center, style: Theme.of(context)
                                  .textTheme
                                  .labelSmall
                                  ?.copyWith(
